...
2. **依頼者の画面が製品。** 依頼者が見る文面は**5種類だけ**。毎回、先頭に「いま／次に誰が動く／あなたがすること／見る場所」を置く。
   専門用語は対訳表の平易語で書く（§5）。
...
│   ├─ smoke.mjs                    ★ 1件通す: テストIssueを立てて plan→注文→終了まで自動で確かめる
...
### 5.2 状況行（全返事の先頭・ラッパーが生成）

```
> 🤖 **Claude（AI）からの返信です**
> **いま**: 対応案を出しました　**次に動く人**: 承認者 @kenhomma（着手OKを付けます）
> **あなたがすること**: 内容を読んで、違うところがあればそのままコメントしてください（@claude は不要）
```

「直しました」では 4行目に **見る場所**: URL を置く。依頼者は先頭3行だけ読めば「どこまで進んで、次に誰が、自分は何を」が分かる。
...
5. **`node scripts/smoke.mjs` で1件通す。** テストIssueを立て、ack→対応案→注文→改訂案→クローズまで自動で確かめる。**通るまで他へ展開しない**

## 7. 段階計画

| 段階 | 何をするか | 終わりの条件 |
|---|---|---|
| Phase 1（このリポ） | エンジンを書く。`demo/` に小さな静的ページを置き、このリポ自身の Issue で全経路を通す | `smoke.mjs` が緑。9経路すべて実測で「通った」 |
| Phase 2（ymy-marketing） | 呼び出し30行に置き換え。未検証6経路を smoke で潰す | 実Issue 1件が起票→反映済みまで通る |
